	...
	*/
	...
	var lists = function () {
	    ...
	    var columns = [
	        ...
	        },
	        {
	            ...
	            "render": function(data, type, full) {
	                var edit =  '<td>';
	                    edit +=  ' <a href="' + url + "preview-print-bni/" +full.upload_id + '" class="btn btn-info btn-circle print-slip" rel="tooltip" title="Print Slip" data-placement="top" data-validate="' + data.upload_id + '"><i class="fa fa-print"></i></a>';
	                    edit +=  ' <a href="' + url + "detail-print-hsbc/" +full.upload_id + '" class="btn btn-warning btn-circle detail" target="_blank" rel="tooltip" title="View Detail Slip" data-placement="top" data-validate="' + data.upload_id + '"><i class="fa fa-eye"></i></a>&nbsp;'; 
	                    if( role_session == "ADMINISTRATOR" || role_session == "SYSTEM") {
							edit += '<a href="'+ url +'delete_bni" data-id ="' + full.upload_id + '" data-name ="' + full.name + '" class="btn btn-danger btn-circle delete-confirm" rel="tooltip" title="Delete Slip" data-placement="top" ><i class="fa fa-trash-o"></i></a>';
						}                              
	                    edit +=  '</td>';
	...
	var listenToPrintWithConfirmation = function() {
	    $(document).on('click', '.print-slip', function(event) {
	        ...
	        if (should_validate) {
	            //get password first.
	            showPasswordConfirm(function(password) {
	                //show loading.
	            ...
	            showPasswordConfirm(function(pass) {
	                ...
	                $.ajax({
	                    ...
	                    success: function(data) {
	                        ...
	                        $('.loading').css("display", "none");
	                        console.log(data);
	                        window.open("<?= site_url('slip/detail-print-bni/'); ?>"+ should_validate);
	                       // location.href = 
	                    },
	                    error: function() {
	                        console.log("error");
