<script type="text/javascript">
	...
	*/
	...
	var lists = function () {
	    ...
	    var ajax_source = "<?= site_url('slip/list_all_data') ?>";
	    ...
	    var columns = [
	        ...
	        },
	        {
	            ...
	            "render": function(data, type, full) {
	                var edit =  '<td>';
	                    edit +=  ' <a href="' + url + "preview_print/" +full.upload_id + '" class="btn btn-info btn-circle print-slip" rel="tooltip" title="Print Slip" data-placement="top" data-validate="' + data.upload_id + '"><i class="fa fa-print"></i></a>';
						edit +=  ' <a href="' + url + "detail_print_aks/" +full.upload_id + '" class="btn btn-warning btn-circle detail" rel="tooltip" target="_blank" title="View Detail Slip" data-placement="top" data-validate="' + full.upload_id + '"><i class="fa fa-eye"></i></a>&nbsp;';
						if( role_session == "ADMINISTRATOR" || role_session == "SYSTEM") {
							edit += '<a href="'+ url +'delete" data-id ="' + full.upload_id + '" data-name ="' + full.name + '" class="btn btn-danger btn-circle delete-confirm" rel="tooltip" title="Delete Slip" data-placement="top" ><i class="fa fa-trash-o"></i></a>';
						}
	                    edit +=  '</td>';
	                return edit;
	            }
	    ...
	    $(document).on("click", ".delete-confirm", function(e) {
	        ...
	        content = 'Apakah anda ingin menghapus slip gaji ' + data_name + ' ?';
	            ...
	            showPasswordConfirm(function(pass) {
	                ...
	                $.ajax({
	                    ...
	                    success: function(data) {
	                        ...
	                        $('.loading').css("display", "none");
	                        console.log(data);
	                        window.open("<?= site_url('slip/detail_print_aks/'); ?>"+ should_validate);
	                       // location.href = 
	                    },
	                    error: function() {
	                        console.log("error");
