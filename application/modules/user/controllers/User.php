...
class User extends CI_Controller  {
    ...
    public function index() 
    {
        ...
        $header = array(
            "title"         => $this->_title,
            "title_page"    => $this->_title_page . '<span>> List STAFF</span>',
    ...
    public function list_char() 
    {
        ...
        $header = array(
            "title"         => $this->_title,
            "title_page"    => $this->_title_page . '<span>> List All Data User CHAR</span>',
            "active_page"   => "user-list-char",
            "breadcrumb"    => $this->_breadcrumb . '<li>User</li>',
        );
        ...
        $footer = array(
            ...
            "view_js_nav" => $this->_view_folder."list_char_js_nav",
        );

        //load the views.
        $this->load->view(MANAGER_HEADER , $header);
        $this->load->view($this->_view_folder . 'index-char');
        $this->load->view(MANAGER_FOOTER , $footer);
    }
    ...
    */
    ...
    public function list_all_data() 
    {
        ...
        //initialize.
        $data_filters   = array();
        $conditions     = array("user_site IS NULL" => NULL);
        $status         = STATUS_ALL;
        ...
        if (count ($filter) > 0) {
            foreach ($filter as $key => $value) {
                ...
                switch ($key) {
                    case 'nik':
                        if ($value != "") {
                            $data_filters['lower(user_nik)'] = $value;
                        }
                        break;
                    ...
                    case 'email':
                        if ($value != "") {
                            $data_filters['lower(user_email)'] = $value;
                        }
                        break;

                    case 'posisi':
                        if ($value != "") {
                            $data_filters['lower(user_position)'] = $value;
                        }
                        break;
    ...
    public function list_all_data_char() 
    {
        ...
        //initialize.
        $data_filters = array();
        $conditions = array("user_site" => "CAR");
        $status = STATUS_ALL;
    ...
    public function process_form() 
    {
        ...
        if ($this->form_validation->run($this) == FALSE) {
            ...
            if( strrpos($username, " ") ) {
                ...
                $message['error_msg'] = "Penigisian username tidak boleh menggunakan spasi";
    ...
    public function process_forms() 
    {
        ...
        if ($this->form_validation->run($this) == FALSE) {
            ...
            if ($id == "") {
                ...
                if( $user_role == 2 ) {
                    $_save_data['user_site'] = 'INTERNAL';
                } else if( $user_role == 4 ) {
                    $_save_data['user_site'] = 'HSBC';
                } else if( $user_role == 5 ) {
                    $_save_data['user_site'] = 'BNI';
                } else {
                    $_save_data['user_site'] = 'DIPO';
                }
                // pr($this->input->post());exit;
                //insert to DB.
                $_save_data['user_created_date'] = $date_now;
                $_save_data['user_password']     = sha1(ENCRYPT_DEV_AKS.$this->db->escape_str($password));
                //then insert
                $result = $this->Dynamic_model->set_model("mst_user","mu","user_id")->insert($_save_data);
                ...
                //condition for update.
                $condition = array("user_id" => $id);
                $result = $this->Dynamic_model->set_model("mst_user","mu","user_id")->update($_save_data, $condition);
    ...
    public function delete_user() 
    {
        ...
        if (empty($id)) {
            ...
            //delete the data (deactivate)
            $condition = array("user_id" => $id);
            $delete = $this->Dynamic_model->set_model("mst_user","mu","user_id")->delete($condition);
     ...
     * Delete an admin.
     */
    public function change_pass_default() 
    {
        ...
        if (empty($id)) {
            ...
            $delete = $this->Dynamic_model->set_model("mst_user","mu","user_id")->update($data,$condition);
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                //growler.
                $message['notif_title']   = "Done!";
                $message['notif_message'] = "Password has been reset (1234)";
                $message['redirect_to']   = "";
            }
        }
        //encoding and returning.
